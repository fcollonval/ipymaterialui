...
class TopComponent extends React.Component {
  ...
  componentWillUnmount() {
    (this.allModels || []).forEach((model) =>
      model.off("change", this.updateCallback)
    );
  }
  ...
  makeProps(model, ancestors) {
    ...
    const childProps = model
      .keys()
      .filter(
        (key) =>
          !key.startsWith("_") && key !== "layout" && model.get(key) != null
      )
      .reduce((props, key) => {
        let propValue;
        if (key === "anchor_el") {
          /* anchor_el prop is an DOM element. Find the DOM element rendered for this model and use is as prop */
          propValue = document.querySelector(
            `[cid=${view.cid}${model.get(key).cid}]`
          );
        } else if (key === "value") {
          ...
          /* Recursive step */
          propValue = this.convertModels(
            model.get(key),
            ancestors.concat(model)
          );
        }
    ...
    if (model.keys().includes("checked")) {
       ...
       */
      const ancestorHandlesEvent =
        (model instanceof RadioModel ||
          model instanceof FormControlLabelModel) &&
       ...
       */
      const ancestorHandlesEvent =
        model instanceof ToggleButtonModel &&
      ...
      childProps.onChange = (e, value) => {
        if (
          model instanceof ToggleButtonGroupModel ||
          model instanceof TabsModel
        ) {
    ...
    (model.get("_events") || []).forEach((eventStr) => {
  ...
  convertModels(value, ancestors) {
    if (value instanceof ReactWidgetModel) {
      return React.createElement(
        value.getReactComponent(),
        this.makeProps(value, ancestors)
      );
    }
    const { view } = this.props;
  ...
  listModels(value) {
    if (value instanceof ReactWidgetModel) {
      const childModels = _.flatten(
        value.values().map((v) => this.listModels(v))
      ).filter(_.identity);
...
export class ReactView extends DOMWidgetView {
  render() {
    super.render();
    ReactDOM.render(this._style(this._render(this.model, this)), this.el);
  }
  ...
  /**
   * Style the provided component
   *
   * @param component Component to be styled
   */
  _style(component) {
    return styleWrapper(component);
  }
}
